...
def get_insta360_gyro_data(path, filterArray=[[1, 0.35], [1,0.04]]):
    fin = open(str(path), "rb")

    gyro_data, exposure_timestamps = _extract_metadata(fin)
    gyro_data = _truncate_gyro_data(gyro_data, exposure_timestamps)
...
def _extract_metadata(fin):
    ...
    while offset > -trailer_len:
        ...
        if hid == '0x300': # Acelerometer data.
            d_times, gyro_data = __parseAccRecord(fin, size)
        if hid == '0x400': # exposure data
            expo_timestamps = __parseExpRecord(fin, size)
        offset = offset - size - 4 - 2
    return gyro_data, expo_timestamps

def _truncate_gyro_data(gyro_data, exposure_timestamps):
...
def __parseAccRecord(fin, size):  
    ...
    for i in range(0, size, dlen):
        ...
        d_gyros.append((tm, -gyroPitch, -gyroYaw, gyroRoll))
